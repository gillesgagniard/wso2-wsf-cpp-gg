...
bool Options::setNTLMProxyAuthInfo(std::string username, std::string password, int flags,
								   std::string domain, std::string workstation,
								   std::string authType)
{
	axis2_status_t status = AXIS2_FAILURE;
	if(_wsf_options)
	{
		status = axis2_options_set_ntlm_proxy_auth_info(_wsf_options, Environment::getEnv(),
			username.c_str(), password.c_str(), flags, domain.c_str(), workstation.c_str(),
			authType.c_str());
		return status ? true : false;
	}
	return false;
...
bool WSF_CALL
Options::setNTLMHTTPAuthInfo(std::string username, std::string password, int flags,
					std::string domain, std::string workstation, std::string authType)
{
	axis2_status_t status = AXIS2_FAILURE;
	if(_wsf_options)
	{
		status = axis2_options_set_ntlm_http_auth_info(_wsf_options, Environment::getEnv(),
			username.c_str(), password.c_str(), flags, domain.c_str(), workstation.c_str(),
			authType.c_str());
		return status ? true : false;	
	}
