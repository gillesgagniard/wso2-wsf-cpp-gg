...
#get_status
#httperf --hog --method POST --add-header="Content-type:application/soap+xml;charset=UTF-8``\n``" --wsesslog=1000,0,esb_input2 --max-piped-calls 8 --rate 8 --timeout 60 --server localhost --port 8280 --uri /services/SampleEventSource > esb_result.txt

httperf --hog --method POST --add-header="Content-type:application/soap+xml;charset=UTF-8``\n``" --wsesslog=4,0,esb_input2 --max-piped-calls 1 --rate 1 --timeout 60 --server localhost --port 8281 --uri /services/SampleEventSource > esb_result.txt
